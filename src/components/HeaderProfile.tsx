...
export const HeaderProfile: React.FC<Props> = ({ onSpeak, calmMode }) => {
  ...
  return (
    <div 
      className="text-center pt-6 pb-4 px-4 flex flex-col items-center select-none cursor-pointer group"
    ...
    >
      ...
      <div className="relative mb-4">
        ...
        <div className={`absolute -inset-1.5 rounded-full opacity-80 blur-sm transition-all group-hover:scale-105 ${
          calmMode 
            ? 'bg-[#9EC079]' 
        ...
        <div className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-full p-1 bg-[#FFFDF9] shadow-lg border-4 border-[#104C64] overflow-hidden flex items-center justify-center">
          ...
          <div className="hidden flex-col items-center justify-center text-white">
            <Puzzle className="w-12 h-12 text-[#8CC63F]" />
        ...
        >
          <Puzzle className="w-4 h-4 fill-current text-[#104C64]" />
        </div>
      </div>

      {/* Main Title "NeuroVisual" (Neuro in dark blue, Visual in rainbow gradient) */}
      <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-2 flex items-center justify-center gap-1">
      ...
      {/* Short Bio / Subtitle */}
      <p className="max-w-xs sm:max-w-md text-sm sm:text-base text-[#104C64]/90 font-semibold leading-relaxed mb-3">
        {bioText}
      </p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-1.5 text-[11px] font-bold">
        <span className="px-2.5 py-1 rounded-full bg-[#E54B88]/10 text-[#E54B88] border border-[#E54B88]/20 flex items-center gap-1">
          <Heart className="w-3 h-3 fill-current" />
          TEA y Neurodiversidad
        </span>
        <span className="px-2.5 py-1 rounded-full bg-[#49A2D2]/10 text-[#49A2D2] border border-[#49A2D2]/20">
          Agendas y Rutinas
        </span>
        <span className="px-2.5 py-1 rounded-full bg-[#8CC63F]/15 text-[#104C64] border border-[#8CC63F]/30 flex items-center gap-1">
